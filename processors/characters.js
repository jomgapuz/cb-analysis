...
const start = async () => {
  const address = web3Helper.getCharactersAddress()
  ...
  const insertBatch = async (items, done) => {
    ...
    const bulk = Characters.collection.initializeUnorderedBulkOp()
    ...
    try {
      ...
      logger('success', 'char', 'processed', bulkResult.nUpserted + bulkResult.nModified)
      done()
    } catch (e) {
      logger('error', 'char', 'processor', e.message)
    }
    await start()
  }

  itemQueue.process(5, async (job, done) => {
    logger('info', 'char', 'processor', `Doing job #${job.id}`)
